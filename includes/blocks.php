...
function block_render_callback( $block, $content = '', $is_preview = false  ) {
	...
	if(!is_admin() && file_exists( get_theme_file_path("/template-parts/blocks/block-{$slug}.php") ) ) {
...
// --------------------------------------------
add_filter( 'block_categories_all', 'base_block_categories' );
...
// --------------------------------------------
add_filter( 'allowed_block_types_all', 'base_allowed_block_types', 10, 2 );
...
// --------------------------------------------
// Return Youtube video ID via URL
// --------------------------------------------
